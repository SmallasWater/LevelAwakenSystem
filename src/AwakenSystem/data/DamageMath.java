...
/**
 ...
 物理防御: 玩家收到实体伤害后减免的伤害
 法术防御: 玩家减免的法术伤害
 暴击: 每 6点数值 增加 1%的暴击 几率 如果大于0 则为 1% 暴击效果为增加伤害的 百分比 最大为 50%
 抗暴: 减少敌方的暴击 伤害 每 6点减免 1 暴击伤害
 抗性: 每6点 增加 1 的魔法防御
 穿透: 每6 点减少敌方 1的物理防御-
...
public class DamageMath implements baseAPI{
    ...
    public int getDelDamage_byPVP(Player damager,Player entity,int[] damage){
        ...
        if(this.cause != null){
            if(this.cause.containsKey("暴击")){
                if(defaultAPI.getPlayerFinalAttributeInt(entity,baseAPI.ItemADDType.ANTI_RIOT) > 0){
                    int c = defaultAPI.getPlayerFinalAttributeInt(entity,baseAPI.ItemADDType.ANTI_RIOT) / 6;
                    int a = this.cause.get("暴击");
                    a -= c;
                    if(a < 0){
                        damage_w -= a;
                    }else{
        ...
        if(!defaultAPI.getPlayerAttributeString(damager.getName(), baseAPI.PlayerConfigType.ATTRIBUTE).equals("null")){
            if(defaultAPI.getPlayerFinalAttributeInt(entity, baseAPI.ItemADDType.RESISTANCE) > 0){
                int c = defaultAPI.getPlayerFinalAttributeInt(damager, baseAPI.ItemADDType.RESISTANCE) / 2;
                int damage_d_e = 0;
                if(c >= 1){
                    damage_d_e = c;
                }
    ...
    public int[] getDamage_byPVP(Player damager,Player entity,float damage){
            ...
            int cRiit = defaultAPI.getPlayerFinalAttributeInt(damager, baseAPI.ItemADDType.CRriT);
            if(cRiit > 0){
                int c = cRiit / 6;
                int damage_b_d = 0;
                if(c >= 1){
